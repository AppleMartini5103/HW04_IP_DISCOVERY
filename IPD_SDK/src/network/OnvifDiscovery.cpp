...
std::string OnvifDiscovery::extractXAddrs(const std::string& xml) {
...
std::string OnvifDiscovery::extractIpFromUrl(const std::string& url) {
    auto start = url.find("://");
...
std::string OnvifDiscovery::httpPost(const std::string& url, const std::string& body, int timeout_ms) {
...
bool OnvifDiscovery::discover(int timeout_ms, std::vector<OnvifDevice>& devices) {
    devices.clear();
    ...
    while (true) {
        ...
        std::string xaddrs = extractXAddrs(xml);

        if (!xaddrs.empty()) {
            OnvifDevice dev;
            dev.service_url = xaddrs;
            dev.ip = extractIpFromUrl(xaddrs);
